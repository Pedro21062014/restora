...
// ===== OPTIMIZATION: Thread pool limited to 2 threads for low RAM =====
fn init_thread_pool() {
    // Limit to 2 threads to keep RAM usage low (~200MB max for rayon)
    rayon::ThreadPoolBuilder::new()
        .num_threads(2)
        .stack_size(512 * 1024) // 512KB per thread stack (default is 2MB)
        .build_global()
...
fn is_thumbnail_file(path: &str, size: u64) -> bool {
    let p = path.to_lowercase();
    let thumb_patterns = ["thumb", "thumbnail", ".thumb", "tn_", "_tn", "preview", "icon", "cache", ".tmp", "~$"];
    if thumb_patterns.iter().any(|pat| p.contains(pat)) { return true; }
    if size < 5_000 && (p.ends_with(".jpg") || p.ends_with(".png")) { return true; }
    let thumb_dirs = [".thumbnails", "thumbs", "thumbnails", "@__thumb"];
    thumb_dirs.iter().any(|dir| p.contains(dir))
}

// ===== OPTIMIZATION: Read only header bytes, not full file =====
...
fn check_header_match(header: &[u8], sig: &FileSignature) -> bool {
    ...
    header[start..end] == *sig.magic
}

fn check_file_integrity_fast(path: &Path, sig: &FileSignature) -> bool {
    // Only check footer for files that have one defined
...
// ===== OPTIMIZATION: Parallel deep scan with deduplication =====
pub fn fast_scan(config: &ScanConfig) -> Vec<RecoveredFile> {
    init_thread_pool();
    let signatures = Arc::new(get_signatures());
    let search_dirs = get_recovery_directories(&config.drive_path);

    if search_dirs.is_empty() {
        return deep_scan_optimized(config);
    }

    // Collect all file paths first (fast, no I/O beyond readdir)
    let mut file_paths: Vec<(PathBuf, u64, String)> = Vec::new();

    for dir in &search_dirs {
        if !Path::new(dir).exists() { continue; }

        for entry in WalkDir::new(dir).max_depth(4).follow_links(false) {
            ...
            file_paths.push((path, size, ext));
        }
    }

    // ===== OPTIMIZATION: Parallel processing with rayon =====
    let config = Arc::new(config.clone());
    let results: Vec<RecoveredFile> = file_paths
        .par_iter()
        .filter_map(|(path, size, ext)| {
            let path_str = path.to_string_lossy().to_string();

            if config.filter_thumbnails && is_thumbnail_file(&path_str, *size) {
                return None;
            }

            // Read header for signature verification
            ...
            for sig in signatures.iter() {
                if check_header_match(&header[..bytes_read], sig) {
                    let is_damaged = check_file_integrity_fast(path, sig);
...
fn deep_scan_optimized(config: &ScanConfig) -> Vec<RecoveredFile> {
    ...
    // Phase 1: Collect file paths (minimal memory, no file opens)
    let mut file_entries: Vec<(PathBuf, u64, String)> = Vec::new();
    let max_depth = if config.scan_type == "deep" { 8 } else { 4 };

    for entry in WalkDir::new(&config.drive_path)
        .max_depth(max_depth)
        .follow_links(false)
        .into_iter()
        .filter_map(|e| e.ok())
    {
        ...
        // Skip active file locations early
        ...
        file_entries.push((path, size, ext));
    }

    // Phase 2: Parallel signature matching
    let config = Arc::new(config.clone());
    let seen_paths = Arc::new(Mutex::new(HashSet::<String>::new()));

    let results: Vec<RecoveredFile> = file_entries
        .par_iter()
        .filter_map(|(path, size, ext)| {
            let path_str = path.to_string_lossy().to_string();
            ...
            // Category filter (fast check)
            if !config.categories.is_empty() {
                let cat_match = signatures.iter().any(|s|
                    s.extensions.iter().any(|&se| se == ext.as_str())
                    && config.categories.contains(&s.category.to_string())
                );
                if !cat_match { return None; }
            }
...
pub fn recover_files(
    ...
    // ===== OPTIMIZATION: Parallel recovery with limited threads =====
    init_thread_pool();
